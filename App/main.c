...
float S3010_mid=1600;
float S3010_right=1200;
float S3010_left=1900;
float turn_temp=0;
float TurnP=0.8;
float TurnI=0.3;
float TurnD=0.1;
...
float ErrorSD=60;
float ErrorS=0;                 //速度调节系数    在main_menu.H
float ErrorD=500;
float K_m=0.75;
float K_K=2.25;
float MINK=0;
float ref_speed =120;            //参考速度
uint8 flag1=0;
uint8 flag2=0;
uint8 flag3=0;
uint8 flag=0;
...
//    ExpectTurn=(-Error*ErrorT+S3010_mid);
  ...
  {
    if(abs(Error)<3.2)                                               //直道速度，转向控制
    {
     
      ExpectSpeed=ref_speed-ErrorS*Error-ErrorSD*abs(Error-LastError);                        //260
